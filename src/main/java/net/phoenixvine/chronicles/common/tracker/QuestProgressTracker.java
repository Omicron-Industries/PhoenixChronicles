    ...
    private static void sweepLockedQuestsIfDue(Player player, PlayerQuestData data) {
        ...
        for (QuestNode node : QuestTreeRegistry.getAllQuests().values()) {
            ...
            for (QuestTask task : node.getEffectiveTasks(player.getServer(), player)) {
                ...
                } else if (task instanceof FluidRequirementTask t) {
                    if (t.isSticky()) task.isCompletedFor(player);
                } else if (task instanceof TagItemTask t) {
                    if (t.isSticky()) task.isCompletedFor(player);
                } else if (task instanceof FilterItemTask t) {
                    if (t.isSticky()) task.isCompletedFor(player);
                } else if (task instanceof FilterFluidTask t) {
                    if (t.isSticky()) task.isCompletedFor(player);
                }
            }
        }
