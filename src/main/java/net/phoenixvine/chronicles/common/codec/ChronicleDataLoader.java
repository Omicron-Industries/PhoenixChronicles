...
import net.phoenixvine.chronicles.common.event.ChronicleEvents;
                        ...
                        switch (type) {
                            ...
                            case "energy_check" -> {
                                ...
                                String srcStr = taskJson.has("source") ? taskJson.get("source").getAsString() :
                                        "INVENTORY";
                                EnergyStorageTask.EnergyType eType;
                                EnergyStorageTask.Source eSrc;
                                try {
                                    eType = EnergyStorageTask.EnergyType
                                            .valueOf(typeStr.toUpperCase());
                                } catch (Exception ignored) {
                                    eType = EnergyStorageTask.EnergyType.FE;
                                }
                                try {
                                    eSrc = EnergyStorageTask.Source
                                            .valueOf(srcStr.toUpperCase());
                                } catch (Exception ignored) {
                                    eSrc = EnergyStorageTask.Source.INVENTORY;
                                }
                                addedTask = new EnergyStorageTask(
