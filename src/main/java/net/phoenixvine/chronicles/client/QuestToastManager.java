...
public class QuestToastManager {
    ...
    // Shrunk from 200x32 - these were taking up a visibly large chunk of the screen, closer to
    // vanilla's own compact advancement-toast proportions instead of a full banner.
    ...
    private static final int STAY_TICKS = 60; // 3 s at 20 tps
    ...
    public void render(GuiGraphics g, int screenW, int screenH) {
        ...
        for (ActiveToast t : active) {
            ...
            int availW = TOAST_W - (textX - x) - 6;
            String label = (t.entry.type == ToastType.COMPLETED) ? "Quest Complete!" : "Quest Unlocked";
            g.drawString(font, "§7" + label, textX, y + 3, (C_LABEL & 0x00FFFFFF) | a, false);
            ...
            g.drawString(font, titleStr, textX, y + 13, (titleCol & 0x00FFFFFF) | a, false);
        }
    }
